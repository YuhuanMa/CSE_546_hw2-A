...
class BinaryLogReg:
    ...
    def __init__(self, _lambda: float = 1e-3):
        ...
        self.weight = np.zeros(784)  # type: ignore
        self.bias: float = 0.0
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        de = np.exp(-y * (self.bias + X @ (self.weight)))
        return 1 / (1 + de)
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        loss = np.mean(np.log((1+np.exp(-y * (self.bias + X @ (self.weight))))))
        reg = self._lambda * self.weight @ self.weight
        return (loss +reg)
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        out = np.mean(((1-self.mu(X,y))*-y)[:,None]* X, axis=0) + 2*self.weight*self._lambda
        return out
    ...
    def train(
        ...
        count = 0
        weight = self.weight - 1 # to start the while loop
        while np.linalg.norm(self.weight - weight) >= 1e-4 and count <= epochs:
...
if __name__ == "__main__":
    ...
    # part b/c/d change for batch_size
    learning_rate = 0.1
    epochs = 100
    batch_size = 100
